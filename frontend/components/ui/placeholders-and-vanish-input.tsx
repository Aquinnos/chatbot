...
export function PlaceholdersAndVanishInput({
    ...
    const animateFrame = (pos: number = 0) => {
      requestAnimationFrame(() => {
        ...
        if (newDataRef.current.length > 0) {
          ...
          // Notify the parent component that the text was sent
          onChange({
            target: { value: '' },
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && !animating && localValue.trim()) {
      e.preventDefault();
      // Call the form submission to send the message
      ...
      >
        ...
        >
          ...
          <motion.path
            ...
            animate={{
              strokeDashoffset: localValue ? 0 : '50%',
            }}
            transition={{
              duration: 0.3,
              ease: 'linear',
